...
public class PartiQLDataModel {
    ...
    */
    ...
    public static long getLongFromStruct(ExprValue node, String name) throws IllegalArgumentException {
        IonStruct struct = getStruct(node);
        if (matchesKeyAndTypeInStruct(struct, name, IonType.INT)) {
            return ((IonInt) struct.get(name)).longValue();
        }
     ...
     * @throws IllegalArgumentException
     */
    public static float getFloatFromStruct(ExprValue node, String name) throws IllegalArgumentException {
    ...
    public static BigDecimal getBigDecimalFromStruct(ExprValue node, String name) throws IllegalArgumentException {
        IonStruct struct = getStruct(node);
        if (matchesKeyAndTypeInStruct(struct, name, IonType.DECIMAL)) {
            return ((IonDecimal) struct.get(name)).bigDecimalValue();
        }

        throw new IllegalArgumentException("Value did not match IonType.DECIMAL");
    }
    public static BigDecimal getDecimalFromStruct(ExprValue node, String name) throws IllegalArgumentException {
        if (node.getIonValue() instanceof IonDecimal ) {
            return ((IonDecimal) node.getIonValue()).bigDecimalValue();
        } else {
            throw new IllegalArgumentException("ionValue did not equal IonDouble?");
        }
    }
